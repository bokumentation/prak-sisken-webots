...
#define TIME_STEP 32
#define MAX_SPEED 3.28
#define BASE_SPEED 2.8
...
int main() {
  ...
  // ================= SENSOR =================
  const char *ir_names[8] = {"IR7", "IR6", "IR5", "IR4",
                             "IR3", "IR2", "IR1", "IR0"};
  DistanceSensor *ir[8];
  ...
  while (robot->step(TIME_STEP) != -1) {
    ...
    // limit motor
    left_speed = std::min(left_speed, max_speed);
    left_speed = std::max(left_speed, -max_speed);

    right_speed = std::min(right_speed, max_speed);
    right_speed = std::max(right_speed, -max_speed);
    ...
    cout << "| L:" << left_speed << " R:" << right_speed << " Error:" << error
         << endl;
  }
